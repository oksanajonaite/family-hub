...
## Problema #12 — `NotificationController`: `catch(Exception e)` + `CustomUserDetails` service'e + 2 DB užklausos

### Kas buvo blogai

```java
// NotificationService — CustomUserDetails visuose metoduose:
...
// Visi naudoja tik currentUser.getId() — kam tuomet visas objektas?

// NotificationController — 2 DB užklausos vietoje 1:
List<NotificationResponse> notifications = notificationService.getMyNotifications(currentUser);
long unreadCount = notificationService.countUnread(currentUser); // ← atskira DB COUNT užklausa

// NotificationController — catch(Exception e):
try {
    notificationService.markAsRead(id, currentUser);
} catch (Exception e) {  // ← sulaiko viską
    redirectAttributes.addFlashAttribute("errorMessage", e.getMessage());
}
```
...
```java
// NotificationService — priima Long userId:
public List<NotificationResponse> getMyNotifications(Long userId)
public void markAsRead(Long notificationId, Long userId)
public void markAllAsRead(Long userId)
// countUnread() pašalintas — nebereikalingas

// NotificationController — 1 DB užklausa + filtravimas atmintyje:
...
// NotificationResponse turi read lauką → skaičiuojame be papildomos užklausos

// NotificationController — konkretūs exceptions:
} catch (NotificationNotFoundException | AccessDeniedException e) {
```

### Pamoka

> **`countUnread()` kaip atskira DB užklausa buvo perteklinė:**
> Jei DTO jau turi `read` lauką — skaičiuok atmintyje iš jau gauto sąrašo.
> Papildoma DB užklausa tik dėl skaičiaus kuris jau yra duomenyse = švaistymas.
>
> **Service metodų parametrų principas (apibendrinimas):**
> Service metodas turi priimti kuo paprastesnius tipus.
> Hierarchija: `Long id` > `String code` > `CustomUserDetails` > `User entity`
> `CustomUserDetails` ir `User entity` service'e reiškia kad service'as priklauso nuo
> sluoksnio kuris jam nepriklauso (Security arba Persistence).
...
| 12 | `catch(Exception e)` + `CustomUserDetails` service'e + 2 DB užklausos vietoje 1 | Exception handling + Performance | `NotificationController` |
